...
def parse_args():
    ...
    # Add a hyperparamter for the item features
    parser.add_argument('--mix_lambda', default=0.2, type=float, help='Paramter for mixing features')
...
class QNetwork:
    def __init__(self, hidden_size, learning_rate, item_num, state_size, pretrain, item_features, _lambda, name='DQNetwork'):   #### modified for adding features
        # Modified for tensorflow v2
        ...
        self.name = name
        self.item_features = item_features  #### modified for adding features
        self._lambda = _lambda  #### modified for adding features
        ...
        with tf.compat.v1.variable_scope(self.name):
            ...
            # final logits
            self.output3 = (1-self._lambda)*self.output2 + self._lambda*self.output3
            ###################################################################################################################
            #### modification end
            ###################################################################################################################
            ...
            ce_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.actions, logits=self.output3)  #### modified for adding features
...
if __name__ == '__main__':
    ...
    #### modified for adding features
    item_features = pd.read_csv(os.path.join(data_directory, "item_features.csv")).to_numpy(dtype=np.float32)

    tf.compat.v1.reset_default_graph()

    #### modified for adding features
    QN_1 = QNetwork(name='QN_1', hidden_size=args.hidden_factor, learning_rate=args.lr, item_num=item_num,
                    state_size=state_size, item_features=item_features, _lambda=args.mix_lambda, pretrain=False)
    #### modified for adding features
